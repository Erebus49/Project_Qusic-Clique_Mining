...
static thread_local struct tms start_time;
...
int WSCP::compare(int set1, int set2, int flag) {
    ...
    if(mult1 == mult2) {
        ...
        if(re == 0)
            return (random() % 3) - 1; //(rand() % 3) - 1 ; //rng.next(3) - 1;
        return re;
    }
...
void WSCP::init()
{
    ...
    int sorted_idx = 0;
    while (uncover_stack_fill_pointer > 0 && sorted_idx < var_num)
    {
	    	int selected_uncover_var = var_sort[sorted_idx++];
	    	if (cover_count[selected_uncover_var] != 0) continue;
    ...
    // Keep a valid incumbent from initialization to avoid empty-best edge cases.
    best_cost = cur_cost;
    best_time = 0;
    for (int i = 0; i < set_num; ++i)
...
int WSCP::select_set_1()
{
    ...
    for(int i = 1;i <= 100;i++)
    {
        ...
        best_set = set_solu[random() % set_count]; //set_solu[rand() % set_count];
...
int WSCP::select_set(int is_tabu)
{
    ...
    if (bms_t < set_count)
    {
        ...
        {
            i = set_solu[random() % set_count]; //set_solu[rand() % set_count];
            if (!cur_solu[i])
                continue;
            if (fix[i])
                continue;
...
int WSCP::select_set_from_zero_stack_new()
{
    ...
    if (bms_t < zero_stack_fill_pointer)
    {
        ...
        {
            tem_set = zero_stack[random() % zero_stack_fill_pointer]; //zero_stack[rand() % zero_stack_fill_pointer];
...
int WSCP::count_2cc(int var)
{
	...
	for(int i = 0;i < 50;i++)
	{
		int cur_set = var_set[var][random() % num];
		if (cc[cur_set] == 2) ans++;
	}
	return ans;
}
...
{
	int var1 = uncover_stack[random() % uncover_stack_fill_pointer];
    ...
    if(!flag2 && !flag3) 
    {
        ...
        {
            int var2 = uncover_stack[random() % uncover_stack_fill_pointer];
            while (var1 == var2) var2 = uncover_stack[random() % uncover_stack_fill_pointer];
            int count1 = count_2cc(var1);
            int count2 = count_2cc(var2);
            double p_cc = (double)count2 / (double)(count1 + count2);
            if ((random() % 100)/(double)101 < p_cc) var1 = var2;
        }
    }
	/*if (uncover_stack_fill_pointer != 1)
	{
    ...
    for (int i = 0; i < var_set_num[selected_uncover_var]; ++i)
    {
        ...
        else if ((this->*compareVersion)(sr, ct, score[cur_set], cost[cur_set], ps, pscore[cur_set], weight1, weight2) == 0)
        {
            ...
            if(random() % 2 == 0) best_set = cur_set;
            /*if (pscore[best_set] < pscore[cur_set])
                best_set = cur_set;
            else if (pscore[best_set] == pscore[cur_set] && time_stamp[best_set] > time_stamp[cur_set])
                best_set = cur_set;*/
    ...
    for (int i = 0; i < var_set_num[selected_uncover_var]; ++i)
    {
        ...
        else if ((this->*compareVersion)(sr, ct, score[cur_set], cost[cur_set], ps, pscore[cur_set], weight1, weight2) == 0)
        {
            ...
            if(random() % 2 == 0) sec_best_set = cur_set; 
    ...
    if (sec_best_set != -1)
    {
        ...
        if ((random() % 100)/(double)101 < pp * novelty_p)
...
int WSCP::local_greedy_addition() {
    ...
    int selected_uncover_var = uncover_stack[random() % uncover_stack_fill_pointer]; //uncover_stack[rand() % uncover_stack_fill_pointer];
    ...
    if(total_num < BMS_T) {
        ...
        for(int i=0;i<BMS_T;i++) {
            cur_set = var_set[selected_uncover_var][random()%total_num];
            if(best_set == -1) best_set = cur_set;
            if (step - time_stamp[cur_set] < tabu_length)
                continue;
            if(compare(cur_set, best_set, 1) > 0)
...
void WSCP::local_search1()
{
    ...
    else t = (random() % (int)(0.1 * set_count)) + 0.2 * set_count; //(rand() % (int)(0.1 * set_count)) + 0.2 * set_count;
    ...
    while(cnt <= t)
    {
        ...
        if (var_n < 0) break;
        int selected_var = set_var[selected_set][random() % var_n]; //set_var[selected_set][rand() % var_n];
            ...
            if (uncover_stack_fill_pointer > 0)
            {
                ...
                if (cur_cost + cost[best_set] + gap >= best_cost || flag_mode1)
                {
                    ...
                    else
                    {
                        ...
                        if(double(random() % 101) / 100.0 < 0.5) flag_1 = 1;
                    	//printf("%lld\n",step);
                    	flag_mode1 = 1;
                    	//step = 0;
                        if (flag4|| flag6) flip_set = remove_best_score(); 
...
int WSCP::remove_best_score() {
    ...
    if(bms_t < set_count) {
        for (int j = 0; j < bms_t; ++j)
        {
            cur_set = set_solu[random()%set_count]; //set_solu[rand()%set_count]; //set_solu[rng.next(set_count)];
