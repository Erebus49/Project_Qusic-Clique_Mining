...
WSCP::WSCP()
{
    ...
    cover_set = nullptr;
    cover_set2 = nullptr;
    uncover_stack = nullptr;
    index_in_uncover_stack = nullptr;
    unlocked_stack = nullptr;
    cur_solu = nullptr;
    best_solu = nullptr;
    set_solu = nullptr;
    index_in_set_solu = nullptr;
    var_delete = nullptr;
    set_delete = nullptr;
    var_set = nullptr;
    var_set_num = nullptr;
    org_var_set_num = nullptr;
    set_var = nullptr;
    set_var_num = nullptr;
    org_set_var_num = nullptr;
    goodset_stack = nullptr;
    last_flip = nullptr;
    last_unflip = nullptr;
    last_flip_num = 0;
    last_unflip_num = 0;
...
void WSCP::set_param()
{
    ...
    if (/*(var_num > 100 && var_num < 150 && set_num > 150 && set_num < 200) ||
        ...
        (var_num > 600 && var_num < 650 && set_num > 3450 && set_num < 3500) || (var_num > 2800 && var_num < 2850 && set_num > 13400 && set_num < 13500) || (var_num > 19100 && var_num < 19200 && set_num > 62500 && set_num < 62600) || (var_num > 74000 && var_num < 75000 && set_num > 166000 && set_num < 167000))
        flag5 = 1;
...
void WSCP::all_memory()
{
    ...
    last_flip = new int[max_set_num];
    last_unflip = new int[max_set_num];
    last_flip_num = 0;
    last_unflip_num = 0;
    owns_memory = true;
}
...
void WSCP::flip(int flip_set)
{
    ...
    if (cur_solu[flip_set] == 1) {
        ...
        for (int i = 0; i < set_var_num[flip_set]; ++i) {
            ...
            if (!flag2 && !flag3)
                for (int j = 0; j < var_set_num[cur_var]; ++j) {
                    if (cc[var_set[cur_var][j]] >= 1)
                        cc[var_set[cur_var][j]] = 2;
                    else
                        cc[var_set[cur_var][j]] = 1;
                }
            /*for (int j = 0; j < var_set_num[cur_var]; ++j)
...
void WSCP::init()
{
    ...
    best_cost = INT_MAX;
    unlocked_stack_fill_pointer = 0;
    index_in_unlocked_stack.assign(set_num, -1);
    ...
    for (int i = 0; uncover_stack_fill_pointer > 0; i++) {
        ...
        for (int j = 0; j < var_set_num[selected_uncover_var]; ++j) {
            ...
            if (fix[cur_set] == -1 || fix[cur_set] == 1)
                continue;
            int cmp_res = (sr == INT_MIN) ? -1 : (this->*compareVersion)(sr, ct, score[cur_set], cost[cur_set], ps, pscore[cur_set], weight1, weight2);
            if (sr == INT_MIN || cmp_res < 0) {
                sr = score[cur_set];
                ps = pscore[cur_set];
                ct = cost[cur_set];
                best_set = cur_set;
...
int WSCP::select_set(int is_tabu)
{
    ...
    if (bms_t < set_count) {
        for (int j = 0; j < bms_t; ++j) {
            ...
            if (fix[i])
                continue;
            int cmp_res = (sr == INT_MIN) ? -1 : (this->*compareVersion)(sr, ct, score[i], cost[i], best_pscore, pscore[i], weight1, weight2);
            if (sr == INT_MIN || cmp_res < 0) {
                sr = score[i];
...
int WSCP::select_set_new(int is_tabu)
{
    ...
    if (bms_t < set_count) {
        for (int j = 0; j < bms_t; ++j) {
            ...
            if (fix[i])
                continue;
            int cmp_res = (sr == INT_MIN) ? -1 : compareV2(sr, ct, score[i], cost[i], best_pscore, pscore[i], weight1, weight2);
            if (sr == INT_MIN || cmp_res < 0) {
                sr = score[i];
                best_pscore = pscore[i];
                ct = cost[i];
...
int WSCP::select_set_Novelty()
{
    ...
    if ((rand_r(&seed) % 100) / (double)101 < pp * novelty_p)
...
int WSCP::select_var_nr(const vector<int>& nr_candidates)
{
    if (var_num <= 0 || set_num <= 0)
        return -1;
    if (!nr_candidates.empty())
        return nr_candidates[rand_r(&seed) % nr_candidates.size()];
    for (int i = 0; i < var_num; ++i) {
        int v = rand_r(&seed) % var_num;
        if (var_delete[v] == 0 && var_set_num[v] > 0)
            return v;
    }
    return -1;
}
...
void WSCP::local_search1()
{
    ...
    if (set_count == 0)
        t = 0;
    else if ((int)(0.1 * set_count) == 0)
        t = 1;
    else
        t = (rand_r(&seed) % (int)(0.1 * set_count)) + 0.2 * set_count; //(rand() % (int)(0.1 * set_count)) + 0.2 * set_count;
    // printf("%d %d ",set_count,t);
    t = std::max(t, 100);
    // printf(" %d\n",t);
    vector<int> nr_candidates;
    if (last_flip_num > 0 && last_unflip_num > 0) {
        vector<char> in_last_flip(var_num, 0);
        vector<char> used(var_num, 0);
        for (int i = 0; i < last_flip_num; ++i) {
            int s = last_flip[i];
            if (s < 0 || s >= set_num)
                continue;
            for (int j = 0; j < set_var_num[s]; ++j) {
                int v = set_var[s][j];
                if (v >= 0 && v < var_num && var_delete[v] == 0 && var_set_num[v] > 0)
                    in_last_flip[v] = 1;
            }
        }
        nr_candidates.reserve(var_num);
        for (int i = 0; i < last_unflip_num; ++i) {
            int s = last_unflip[i];
            if (s < 0 || s >= set_num)
                continue;
            for (int j = 0; j < set_var_num[s]; ++j) {
                int v = set_var[s][j];
                if (v >= 0 && v < var_num && in_last_flip[v] && !used[v] && var_delete[v] == 0 && var_set_num[v] > 0) {
                    used[v] = 1;
                    nr_candidates.push_back(v);
                }
            }
        }
    }
    ...
    while (cnt <= t) {
        ...
        // if (var_n < 0)
        //     break;
        int selected_var = select_var_nr(nr_candidates); // set_var[selected_set][rand_r(&seed) % var_n]; set_var[selected_set][rand() % var_n];
        if (selected_var < 0)
            break;
        if (var_set_num[selected_var] <= 0)
            break;
        int number = 0;
        ...
        if (uncover_stack_fill_pointer == 0) {
            while (zero_stack_fill_pointer > 0) {
                ...
                if (index < unlocked_stack_fill_pointer && unlocked_stack_fill_pointer > 0 && index >= 0) {
                    ...
                    unlocked_stack[index] = top_set;
                    index_in_unlocked_stack[top_set] = index;
                }
            }
        ...
        for (step = 0; step < max_step; ++step) {
            ...
            if (out_nonimpr == OutterDepth) {
                if (gap == 0) {
                    gap = gap_max;
                    zero_time++;
                    if (zero_time == max_zero_time)
                        out_nonimpr_new = OutterDepth_new;
                } else {
                    gap = gap / 2;
                    out_nonimpr = 0;
                }
            ...
            if (uncover_stack_fill_pointer == 0) {
                if (zero_stack_fill_pointer > 0) {
                    ...
                    last_unflip_num = 0;
                    for (int i = 0; i < set_num; ++i) {
                        if (best_solu[i] == 0 && cur_solu[i] == 1)
                            last_flip[last_flip_num++] = i;
                        if (best_solu[i] == 1 && cur_solu[i] == 1)
                            last_unflip[last_unflip_num++] = i;
                        best_solu[i] = cur_solu[i];
                    }
            ...
            if (uncover_stack_fill_pointer > 0) {
                int best_set;
                if (flag5 || flag6)
                    best_set = local_greedy_addition();
                else {
                ...
                if (best_set == -1) {
                    int flip_set = (flag4 || flag6) ? remove_best_score() : select_set(0);
                    if (flip_set == -1)
                        return;
                    flip(flip_set);
                    time_stamp[flip_set] = step;
                ...
                if (cur_cost + cost[best_set] + gap >= best_cost || flag_mode1) {
                    ...
                    if (flip_set == -1) { // printf("2222\n");
                        ...
                        // printf("<%lld>\n",step);
                        flag_mode1 = 0;
                        if (flip_set == -1 || best_set == -1) // printf("3333\n");
                            return;
                        flip(flip_set);
                        flip(best_set);
                        time_stamp[flip_set] = step;
                        ...
                        if ((step != 0) && (step % 1000 == 0)) {
                            if (get_runtime() > cutoff_time) // printf("4444 step %lld time %lf, cutoff_time %lf\n", step, get_runtime(), cutoff_time);
                                return;
                        }
                        continue;
                    } else {
                        // if(double(rand() % 101) / 100.0 < 0.5) flag_1 = 1;
                        if (double(rand_r(&seed) % 101) / 100.0 < 0.5)
                            flag_1 = 1;
                        // printf("%lld\n",step);
                        flag_mode1 = 1;
                        // step = 0;
                        ...
                        if ((step != 0) && (step % 1000 == 0)) {
                            if (get_runtime() > cutoff_time) // printf("7777\n");
                                return;
                        }
                        continue;
                    }
                } else {
                    if (best_set == -1) // printf("8888\n");
                        return;
                    flip(best_set);
                    update_weight();
                    time_stamp[best_set] = step;
                    if ((step != 0) && (step % 1000 == 0)) {
                        if (get_runtime() > cutoff_time) // printf("9999\n");
                            return;
                    }
                    continue;
                }
            ...
            if (flag4 || flag6)
                ...
                flip_set = select_set(0);
            if (flip_set == -1) // printf("101010\n");
                return;
            flip(flip_set);
            time_stamp[flip_set] = step;
            if ((step != 0) && (step % 1000 == 0)) {
                if (get_runtime() > cutoff_time) // printf("121212\n");
                    return;
            }
        }
    }
...
int WSCP::remove_best_score()
{
    ...
    if (bms_t < set_count) {
        for (int j = 0; j < bms_t; ++j) {
            ...
            if (best_set == -1 || compare(cur_set, best_set, -1) > 0)
                best_set = cur_set;
        }
    } else {
        for (int j = 0; j < set_count; ++j) {
